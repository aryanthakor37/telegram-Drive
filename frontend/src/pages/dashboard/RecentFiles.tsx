...
import {
  File, Image, Video, Music, Archive, FileText, Clock, Download, Trash2, Share2,
  AlertCircle, Star, FileCode
} from 'lucide-react';
...
const RecentFiles: React.FC = () => {
  ...
  const fetchFiles = async () => {
    try {
      ...
      setError('');
      const res = await axios.get(`${API_URL}/drive/files/recent`, { withCredentials: true });
      setFiles(res.data);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {!loadingFiles && filteredFiles.length === 0 ? (
        ...
        >
          <div className="w-24 h-24 mb-6 rounded-full bg-slate-500/10 flex items-center justify-center">
            <Clock className="w-12 h-12 text-slate-400" />
          </div>
          <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No recent files</h3>
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">
            Files you upload or interact with recently will appear here.
          </p>
        </motion.div>
      ) : viewMode === 'grid' ? (
